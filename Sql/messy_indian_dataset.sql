...
SELECT count(*) FROM messy_indian_dataset;

select sum(cnt) from (
select distinct name,cnt from(
select name,
		age,
        purchase_amount,lag(purchase_amount) over(partition by name) as lago,
        count(*) over(partition by name) as cnt
from messy_indian_dataset) k) t;


select name,
		age,
        purchase_date,
        lag(purchase_date) over(partition by name) as lago,
        datediff(purchase_date,lag(purchase_date) over(partition by name))  as cnt
